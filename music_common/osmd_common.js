 ...
 */
    ...
    var c_chord_data = {
        "CM7": {notes: ["C", "E", "G", "B"], 
            chord_kind: "major-seventh", tritone: false, leading: false },
        "Cmaj7": {notes: ["C", "E", "G", "B"], 
                chord_kind: "major-seventh", tritone: false, leading: false },
        "CM": { notes: ["C", "E", "G"], 
        ...
        "Cm7b5": {notes: ["C", "Eb", "Gb", "Bb"], 
            chord_kind: "half-diminished", tritone: true, leading: true },
        "Cm7": {notes: ["C", "Eb", "G", "Bb"], 
            chord_kind: "minor-seventh", tritone: false, leading: false },
        "Cmmaj7": {notes: ["C", "Eb", "G", "B"], 
            chord_kind: "major-minor", tritone: false, leading: true },
        "Cm6": {notes: ["C", "Eb", "G", "A"], 
        ...
        "C7b5": {notes: ["C", "E", "Gb", "Bb"], 
            chord_kind: "half-diminished", tritone: true, leading: false },
            
        "C7#5": {notes: ["C", "E", "G#", "Bb"], 
            chord_kind: "", tritone: true, leading: false },
            
        "C7sus4": {notes: ["C", "F", "G", "Bb"], 
            chord_kind: "dominant", tritone: false, leading: false ,
            degree: `
                <degree>
        ...
        "C7add6": {notes: ["C", "E", "A", "Bb"],
            chord_kind: "dominant", tritone: true, leading: true },
            ...
            "C9": {notes: ["C", "E", "G", "Bb", "D"], 
                chord_kind: "dominant-ninth", tritone: true, leading: true },
            "CM9": {notes: ["C", "E", "G", "B", "D"], 
                chord_kind: "major-ninth", tritone: true, leading: true },
            "Cmaj9": {notes: ["C", "E", "G", "B", "D"], 
                    chord_kind: "major-ninth", tritone: true, leading: true },
            "Cm9": {notes: ["C", "Eb", "G", "Bb", "D"], 
                chord_kind: "minor-ninth", tritone: true, leading: true },
    
            "C11": {notes: ["C", "E", "G", "Bb", "D", "F"], 
                chord_kind: "dominant-11th", tritone: true, leading: true },
            "CM11": {notes: ["C", "E", "G", "B", "D", "F"], 
                chord_kind: "major-11th", tritone: true, leading: true },
            "Cmaj11": {notes: ["C", "E", "G", "B", "D", "F"], 
                    chord_kind: "major-11th", tritone: true, leading: true },
            "Cm11": {notes: ["C", "Eb", "G", "Bb", "D", "F"], 
                chord_kind: "minor-11th", tritone: true, leading: true },
    
    
            "Cdim7": {notes: ["C", "Eb", "Gb", "A"], 
                chord_kind: "diminished-seventh", tritone: true, leading: true },
            "Cdim": {notes: ["C", "Eb", "Gb"], 
                chord_kind: "diminished", tritone: false, leading: true },
            "Caug7": {notes: ["C", "E", "G#", "Bb"], 
                chord_kind: "augmented-seventh", tritone: false, leading: false ,
                degree:`
                    <degree>
            ...
            "C9sus4": {notes: ["C", "F", "G", "Bb"], 
                    chord_kind: "dominant-ninth", tritone: false, leading: false ,
                    degree: `
                        <degree>
                        <degree-value>4</degree-value>
                        <degree-alter>0</degree-alter>
...
function make_xml_score_for_chords(is_test)
{
    ...
    function add_note(sbase, octave, half_steps)
    {
        ...
        xml_score += `<note xxcolor="#0000ff" default-x="26">\n`;
        if (half_steps != 0)
        {
            xml_score += `<chord />\n`;
        }
        xml_score += sprintf(`<pitch>
            ...
            <instrument id="P1-I1" />
            <type>quarter</type>\n`, new_note_step, new_octave, new_note_alter);
            
        //xml_score += `<lyric default-y="-80" number="part1verse1" xxcolor="#000000">
...
function get_chords()
{
   ...
   // boundingBox.dataObject.staffEntries[0].lyricsEntries[0].lyricsEntry.text
    ...
    // together
    // measures[2][0].staffEntries[0].graphicalChordContainer.chordSymbolContainer.chordKind
    // measures[2][0].staffEntries[1].lyricsEntries[0].graphicalLyricWord.graphicalLyricsEntries[0].lyricsEntry.text: 'prom'
    //chordKind = measures[2][0].staffEntries[0].graphicalChordContainer.chordSymbolContainer.chordKind;
    //("measures[2][0].staffEntries[0].graphicalChordContainer.chordSymbolContainer.chordKind: %s", chordKind);
    //lyric = measures[2][0].staffEntries[1].lyricsEntries[0].graphicalLyricWord.graphicalLyricsEntries[0].lyricsEntry.text;
    //console.log("measures[2][0].staffEntries[1].lyricsEntries[0].graphicalLyricWord.graphicalLyricsEntries[0].lyricsEntry.text: %s",
    //   lyric);
    ...
    for (im = 0; im < measures.length; im++)
    {
        ...
        //for (imelt = 0; imelt < measures[im].length; imelt++)
        {
            ...
            for (staff_number = 0; staff_number < 1; staff_number++)
            {
                ...
                graphicalChordContainer = staff_entry.graphicalChordContainer;
...
function set_options(start_measure, end_measure, make_transparent, hide_rests, drawing_color)
{
    ...
    rest_color = drawing_color;
    if (hide_rests)
        rest_color = "#00000000";
    
    if (make_transparent)
    {
        drawing_color = "#00000000";
        rest_color = "#00000000";
        if (get_element_value("transparent_color") == "red")
        {
            drawing_color = "#ff0000"; // uncomment to see notes
            rest_color = "#ff0000";
        }
        coloring_enabled = 1;
    }
...
// since this is recursive, all variables need to be let 
...
{   
    ...
    let sindent = level + ": " + "                    ".substr(0, level * 2);
    if (!path)
        path = "";
    ...
    for (let key in obj) 
    {
        let spath = sprintf("%s.%s", path, key);
        if (!isNaN(key))
            spath = sprintf("%s[%s]", path, key);
        let spre = sprintf("%s%s", sindent, spath);
        ...
        for (prop in obj2) {
            ...
            let spath2 = sprintf("%s.%s", spre, prop);
            if (!isNaN(prop))
            ...
            let is_array = Array.isArray(prop_value);
            ...
            if (is_array )
            {
                ...
                for (let ii = 0; ii < prop_value.length && ii < 5; ii++)
                {
                    //console.log("%s SHOW ARRAY ELT: %s: %s",  spre2, ii, prop_value[ii]);
                    let sarray = sprintf("%s[%s]", spath2, ii);
    ...
    {
        let today = new Date();
        let time = sprintf("%02d:%02d:%02d %03dms",
